...
rawdata_dir = '/proj/_docs/Dropbox (MIT)/proj/screencam/rawdata/';
dataset_dir = '../dataset/'; % dataset directory
results_dir = '../results/'; % results

% available quantization levels: 4.0, 4.4, 4.9, 5.2, 5.6 [bits]
quant_level = 4.4; % quantization level - bits
[scfile, measidx, p, rhos, ges_suffix, imindex] = load_vary_quant_levels_data(quant_level);
...
switch lower(csmethod)
    ...
    case 'pnp'
        ...
        para.csmethod = 'PnP-QCS'; % PnP-QCS method
          ...
          para.rho = 1.5e2; % multiplier (noise regularier) [32x32x100%] (als) [0,18] lux
          if exist('p', 'var')
              para.p = p;
          end
          if exist('rhos', 'var')
...
if BRCORRFLAG
    ...
    imnit = iminit.*(1+((X-x0).^2/a^2+(Y-y0).^2/b^2)/d^2);
end
...
set(gca, 'Color', 'white');
imagesc(reshape(Tx_zf,[v h])); axis image; % colormap jet;
  c = colorbar; caxis([-10 50]); c.Ticks=-10:10:50; c.LineWidth=linewidth;
  xticks(10:10:30);yticks(10:10:30);
set(gca,'linewidth',linewidth);

saveas(fig2, [figdir '/fig04' imindex '_vary_quant_levels_transform_domain_' num2str(scfile(1)) '.svg']);

% Figure 2 | raw initialization
...
saveas(fig3, [figdir '/fig04' imindex '_vary_quant_levels_raw_initialization_' num2str(scfile(1)) '.svg']);

% Figure 3 | final reconstruction
...
saveas(fig4, [figdir '/fig04' imindex '_vary_quant_levels_final_recon_' num2str(scfile(1)) '.svg']);
