...
%       sparams.NUMPHSHIFT = 2; % number of phase shifts to get the complex value of the DFT sensing matrix
    ...
    sparams.SAMPLE_BINARY  = true; % sample is binary (1-Y, 0-N)
    ...
    sparams.QUANT_STEPS  = 1e31; % steps of quantization
...
% % %       csparams.sigma   = [50 25 12  6]/MAXB; % noise deviation (to be estimated and adapted)
% % %       csparams.maxiter = [10 10 10 10];
% %       csparams.sigma   = [ 6]/MAXB; % noise deviation (to be estimated and adapted)
% %       csparams.maxiter = [ 1];
% % %       csparams.sigma   = [100 50 25 12  6]/MAXB; % noise deviation (to be estimated and adapted)
...
a = -8;
b = 18;
% Tx_zf_rescale = (Tx_zf-min(Tx_zf))/(max(Tx_zf)-min(Tx_zf))*(b-a)+a;
Tx_zf_rescale = (Tx_zf)/(max(Tx_zf))*b;
% Figure 1 | transform domain
fig2 = figure('position',[50,100,300,250]);
set(gca, 'Color', 'white');
imagesc(reshape(Tx_zf_rescale,[v h])); axis image; % colormap jet;
  c = colorbar; caxis([-10 20]); c.Ticks=-10:10:20; c.LineWidth=linewidth;
  xticks(10:10:30);yticks(10:10:30);
set(gca,'linewidth',linewidth);

saveas(fig2, [figdir '/fig04' imindex '_vary_quant_levels_transform_domain.svg']);

% Figure 2 | raw initialization
fig3 = figure('position',[50,100,300,250]);
set(gca, 'Color', 'white');
% imagesc(imrecon); axis image; colormap('gray');
imagesc(imnorm(reshape(x_zf,[v h]))); axis image; colormap('gray');
  c = colorbar; caxis([0 1]); c.Ticks=0:1; c.LineWidth=linewidth;
  xticks(10:10:30);yticks(10:10:30);
set(gca,'linewidth',linewidth);

saveas(fig3, [figdir '/fig04' imindex '_vary_quant_levels_reference.svg']);
