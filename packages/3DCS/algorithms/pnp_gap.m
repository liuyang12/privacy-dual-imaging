function [ x, psnrall ] = pnp_gap( A, y, opt )
%PNP_GAP Plug-and-play (PnP) algorithm(s) based on generalized alternating 
...
for isig = 1:length(maxiter) % extension for a series of noise levels
    ...
    for iter = 1:maxiter(isig)
        ...
        yb = A*z;
        if acc % accelerated GAP (noise-free only)
            yacc = yacc + (y-yb);
            if orthogonal_basis % [orthogonal basis] A*A' diagonal
                x = z + A'*( (yacc-yb)./d ); % element-wise
            else % [general basis]
                x = z + Apinv*(yacc-yb); % matrix product
            end
        else
            if orthogonal_basis % [orthogonal basis] A*A' diagonal
                x = z + A'*( (y-yb)./d ); % element-wise
            else % general basis
                x = z + Apinv*(y-yb); % matrix product
                % x = z + Apinvy-ApinvA*z; % pre-calc #2
            end
        end
        ...
        if flag_iqa && isfield(opt,'orig') && (~isempty(opt.orig))
            ...
            if (mod(k,5)==0) 
                fprintf('  PnP-GAP-%s iteration % 4d, sigma % 3d, PSNR %2.2f dB.\n',...
