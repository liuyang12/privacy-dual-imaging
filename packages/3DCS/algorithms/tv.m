...
deft.x0      = zeros(size(A,2),1); % [default] start point (pseudo-inverse result)
...
tic; % start stopwatch timer (running time)
...
for it = 0:maxiter % number of iteration count
    ...
    % x = pinvmatH_*(w+1/mu*r)+pinvmatA_*(y+1/mu*s); % sign of y-1/mn*s is wrong
...
if conv % converged at less than the maximum iterations
    fprintf('TV solver converged at its %4d iterations, running time %.2f seconds.\n',it,runtime);
else
    fprintf('TV solver reached the maximum %4d iterations, running time %.f seconds.\n',it,runtime);
end
