...
% scfile      = 1925; % scene file(s), average if multiple
% bgfile      = []; % background file(s), average if multiple
% 
% measidx = [21,26,27,28,29,30]; % index for plotting interested frames
% rhos = [5,6,6,13,9,11]*1e2;
% ges_suffix = '_scroll2_6p';

gesture = 'rotate';
[scfile,measidx,rhos,ges_suffix] = load_gesture_data(gesture);
...
%   denoiser = 'tv';
% % csmethod = 'ap';
% csmethod = 'pnp3d'; % [TODO] normalization before apply video denoising prior
%   denoiser = 'vbm3d';
  % denoiser = 'vbm4d';
  % denoiser = 'bm4d';
...
params.measthresh = 45; % single channel [ambient light sensor output]
...
switch lower(csmethod)
    ...
    case 'pnp'
        ...
        para.csmethod = 'PnP-ADMM'; % PnP-ADMM method
          para.rho = 5e2; % multiplier (noise regularier) [32x32x100%] (als) [0,18] lux
          if exist('rhos', 'var')
              para.rhos = rhos;
          end
